        ...
        <tbody class="font-body text-body divide-y-[2px] divide-outline">
          ...
          <tr v-for="run in runs" :key="run.id" class="hover:bg-surface-container transition-colors">
            ...
            <td class="p-3 border-r-[2px] border-outline">
              <span 
                :class="[
                  statusColors[run.status] || 'bg-surface-dim',
                  run.status === 'Running' ? 'animate-pulse flex items-center gap-1 w-fit' : ''
                ]" 
                class="px-2 py-0.5 border-[2px] border-outline font-label uppercase text-[10px]"
              >
                <span v-if="run.status === 'Running'" class="material-symbols-outlined text-[10px] animate-spin">refresh</span>
                {{ run.status }}
              </span>
            </td>
            ...
            <td class="p-3">
              <div v-if="(run.framework || '').toLowerCase() === 'playwright' && run.status !== 'Running'" class="flex flex-col gap-1">
                <a
                  href="http://127.0.0.1:3000/allure/index.html"
                  target="_blank"
                ...
                <router-link
                  to="/html-report"
                  class="px-2 py-1 bg-[#f97316] text-white font-label uppercase text-[10px] border-[2px] border-outline hover:translate-x-[1px] hover:translate-y-[1px] transition-all flex items-center gap-1 w-fit"
                >
                  <span class="material-symbols-outlined text-[12px]">language</span> HTML
                </router-link>
              </div>
              <div v-else-if="(run.framework || '').toLowerCase() === 'jmeter' && run.status !== 'Running'">
                <router-link
                  to="/jmeter-report"
                  class="px-2 py-1 bg-[#d946ef] text-white font-label uppercase text-[10px] border-[2px] border-outline hover:translate-x-[1px] hover:translate-y-[1px] transition-all flex items-center gap-1 w-fit"
                >
                  <span class="material-symbols-outlined text-[12px]">speed</span> View JTL
                </router-link>
              </div>
